...
public class HydraulicPress extends Machine {
    ...
    public HydraulicPress(Location controllerLocation, AutomationX plugin) {
        super(controllerLocation, "hydraulic_press");
        this.plugin = plugin;
        int baseTicks = plugin.getConfig().getInt("hydraulic-press.base-ticks", 200);
        setTicksRemaining(Math.max(MIN_TICKS, baseTicks));
    }

    @Override
    public boolean isStructureValid() {
        Block controller = getControllerBlock();
        if (controller.getType() != Material.ANVIL &&
            controller.getType() != Material.CHIPPED_ANVIL &&
            controller.getType() != Material.DAMAGED_ANVIL) return false;
    ...
    public boolean tick() {
        ...
        int baseTicks = plugin.getConfig().getInt("hydraulic-press.base-ticks", 200);
        int resetTicks = Math.max(MIN_TICKS, baseTicks);
        ...
        if (cim.hasItem(inputBlock.getLocation())) {
            ...
            if (result == null) {
                setTicksRemaining(resetTicks);
                return false;
            }

            // Determine output conveyor block
            ...
            if (outputBlock.getType() == Material.POLISHED_DEEPSLATE_SLAB) {
                ...
                if (cim.hasItem(outputBlock.getLocation())) {
                    setTicksRemaining(1); // stall until free
                    return false;
                }
        ...
        if (items.isEmpty()) { setTicksRemaining(resetTicks); return false; }
        ...
        if (result == null) { setTicksRemaining(resetTicks); return false; }
        ...
        if (outputBlock.getState() instanceof Chest chest) {
            ...
            if (inv.firstEmpty() != -1) inv.addItem(result);
            else inputLoc.getWorld().dropItemNaturally(inputLoc, result);
        } else {
    ...
    private ItemStack getPressResult(ItemStack input) {
        ...
        if ("steel_ingot".equals(axId))
            return plugin.getItemRegistry().get("steel_plate").createItem(1);
    ...
    public String[] getStats() {
        ...
        int baseTicks = plugin.getConfig().getInt("hydraulic-press.base-ticks", 200);
        int effective = Math.max(MIN_TICKS, baseTicks);
        return new String[]{
            "§6=== Hydraulic Press ===",
            "§7Status: " + (getTicksRemaining() > 0 ? "§aProcessing" : "§7Idle"),
            "§7Power: "  + (hasPower ? "§a✔ Connected" : "§c✘ No power (coming soon)"),
            "§7Speed: §e" + (effective / 20) + "s per item §8(min 3s)",
            "§7Power type: §eElectrical",
            "§7Last processed:",
            "§8  1. §f" + last[0],
            "§8  2. §f" + last[1],
            "§8  3. §f" + last[2]
